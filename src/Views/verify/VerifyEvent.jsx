...
export default class VerifyEvent extends Component {
  render() {
    ...
    return (
      <div
        className={cn(align.full, "py-4", align.topCenter, align.noMarginPad)}
      >
        <Row
          className={cn(align.full, "py-3", align.noMarginPad, align.allCenter)}
        >
          <Col xs="10" className={cn(align.topCenter, align.noMarginPad)}>
            <Verify
              eventID={event.id}
              tokenID={tokenID}
              className={cn("animated", verifyInOut)}
            />
          </Col>
        </Row>
        ...
        <Row className={cn(align.full, align.noMarginPad, align.allCenter)}>
          <Col xs="10" className={cn(align.allCenter, align.noMarginPad)}>
            <Instructions
              {...this.props}
              className={cn("animated", instrInOut)}
            />
          </Col>
        </Row>
      </div>
